...
## 0. 本轮复核与处理结果（2026-06-30）

本轮对审计意见进行了复核。结论是：P0 和大部分 P1 意见合理，均能从 `docs/spec.md`、`docs/design.md`、`docs/development/state-and-errors.md` 或当前原型源码中找到依据；P2 主要属于体验增强，不应阻塞阶段性交付。

### 0.1 合理性评估

| 审计项 | 复核结论 | 本轮处理 |
| --- | --- | --- |
| 任务状态机与开发文档不一致 | 成立。`state-and-errors.md` 明确要求 `preparing -> deduping -> pending_start -> backing_up`，原型脚本此前跳过 `deduping`。 | 已修正 `prototype/assets/prototype.js` 状态流转，并补充烟测断言。 |
| 空状态、加载状态和错误状态不足 | 成立。`docs/spec.md` 明确要求各页面覆盖空、加载、错误状态。 | 已在任务、记录、设备、网盘、去重、任务详情等页面补充样例。 |
| 首次引导阻断感不够强 | 成立。未授权和未绑定时仍把“进入工作台”呈现得过强。 | 已弱化为“稍后查看账号信息”，并增加“当前还差 2 步可开始备份”。 |
| 登录注册错误态不足 | 成立。原型缺少账号不存在、密码错误、网络异常、协议未勾选、验证码错误、密码不一致、账号已存在等样例。 | 已补充登录和注册错误态说明。 |
| 任务列表操作权限不够贴合状态矩阵 | 成立。原型此前未完整覆盖准备中、对比去重中、恢复中、备份失败、已删除等状态。 | 已补齐任务状态样例和状态操作矩阵。 |
| 任务详情恢复解释仍可加强 | 成立。异常恢复场景需要更直观说明已完成、已跳过、将继续和异常待处理内容。 | 已增加“恢复摘要”和“恢复失败”模块。 |
| 备份记录状态和操作不足 | 成立。此前偏查询表格，缺少失败记录、删除前记录和空状态。 | 已增加查看详情、失败记录、任务记录已删除但历史文件保留、暂无备份记录。 |
| 百度网盘授权状态样例不足 | 成立。授权失效是恢复前置条件，需要可视样例。 | 已补充未授权、已授权、授权已失效、授权失败、已解除绑定。 |
| 设备重命名和绑定失败态不足 | 成立。重命名按钮此前没有编辑态，绑定失败缺少可验收样例。 | 已补充重命名中、未绑定设备、设备绑定失败和重试入口。 |
| 品牌记忆点偏弱 | 部分成立。它不影响验收，但可提升产品识别性。 | 已轻量升级品牌标识为云盘、设备节点和锁的抽象符号；完整图标体系留待后续视觉专项。 |
| 主行动层级仍可加强 | 成立但非阻断。 | 已调整工作台行动规则、创建任务主按钮和去重页底部确认区。 |
| 窄屏桌面窗口体验可提升 | 成立但需要截图级 QA 才能完全确认。 | 已增加窄屏表格卡片化样式；仍建议后续做多宽度截图检查。 |

### 0.2 当前仍不足的地方

1. P2 的完整轻量图标体系尚未系统化，只完成了品牌符号增强。
2. 窄屏窗口体验已补 CSS 策略，但仍需要后续在 360px、860px、1280px 等宽度下做截图级视觉 QA。
3. 当前原型仍是静态 HTML 交互样例，没有真实接口、真实文件选择器或真实百度网盘授权流程；这符合当前“原型开发”阶段，但进入实现阶段前需要把状态机和错误码转成组件级与端到端测试。

### 0.3 本轮新增验证口径

`tests/prototype-smoke.test.mjs` 已补充以下断言：

- 状态机覆盖 `deduping`、`start_after_confirm`、`recovery_check_passed`、`recovery_check_failed`、`delete_record`。
- 产品屏幕覆盖任务完整状态样例、空状态、加载状态、错误状态。
- 登录注册、备份记录、授权状态、设备重命名和绑定失败样例均落到对应页面。
...
说明：本轮已通过烟测覆盖目录结构、状态机、关键文案、高影响确认、空/加载/错误态和敏感数据扫描。截图级视觉 QA 仍建议在后续单独执行，重点检查窄屏窗口下导航、表格和按钮的可读性。
